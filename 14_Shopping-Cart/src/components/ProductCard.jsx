...
const ProductCard = ({product, anyHovered}) => {
    ...
    return (
        <div className={`group relative p-4 flex flex-col justify-center items-center bg-primary border rounded-lg drop-shadow-lg ${ !isHovered ? 'opacity-50' : (anyHovered ? 'opacity-100' : null)}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
            <img src={product.image} alt={product.title} className="z-50 w-48 h-48 overflow-hidden border rounded-sm" />
